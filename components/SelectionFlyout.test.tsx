...
describe('SelectionFlyout', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(clearAllSelections).mockReturnValue({ type: 'selectedPokemon/clearAllSelections', payload: undefined });
  });
  ...
  it('renders buttons when Pokemon are selected', () => {
    vi.mocked(useAppSelector).mockImplementation((selector) => {
      ...
      if (selector === selectSelectedPokemonItems) return {
  ...
  it('calls clearAllSelections when remove button is clicked', () => {
    ...
    render(<SelectionFlyout />);
    const removeButton = screen.getByAltText('Remove all items').closest('button');
    if (removeButton) {
      fireEvent.click(removeButton);
    }
    
    expect(mockDispatch).toHaveBeenCalledWith({ type: 'selectedPokemon/clearAllSelections', payload: undefined });
  });
  ...
  it('initiates download when download button is clicked', () => {
    ...
    render(<SelectionFlyout />);
    const downloadButton = screen.getByAltText('Download CSV').closest('button');
    if (downloadButton) {
